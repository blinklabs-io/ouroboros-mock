...
This package provides a shared harness for Cardano ledger-rule vectors. The
corpus is pinned to a Cardano Blueprint archive tracked at
`blueprint-vectors.tar.gz` and refreshed from the Git submodule; see
[CORPUS.md](CORPUS.md) for provenance, refresh, and coverage boundaries.

It can be used by any implementation that provides the required state
interfaces. Consensus, protocol-wire, and synthetic rollback fixtures are
separate layers and are not represented by the ledger corpus totals below.
...
| Metric | Count |
|--------|-------|
| Total vector files | 2,574 |
| Protocol parameter files | 78 |
| Results | Reported per vector and rule family by the test run |

The verbose conformance CI job prints a deterministic coverage table grouped
by ledger era and Blueprint rule family. Each row includes total, passed, and
failed vectors. The table describes this mock backend's exercised corpus; it
is not a claim that every Cardano ledger rule is implemented.

The working copy under `testdata/eras` is generated by
`make prepare-blueprint-testdata` and is not tracked. The tracked form is the
pinned archive `blueprint-vectors.tar.gz`, which is what module consumers
receive; `ExtractEmbeddedTestdata` extracts it alongside the synthetic corpus.
...
**Source**: Cardano Blueprint vectors from
[cardano-scaling/cardano-blueprint](https://github.com/cardano-scaling/cardano-blueprint/tree/main/src/ledger/conformance-test-vectors)

The vectors are stored in `testdata/eras/conway/impl/dump/` and include:
- **2,574 test vector files** (Blueprint JSON records containing exact CBOR hex)
- **78 protocol parameter files** in `pparams-by-hash/`
...
Reward balances come from initial state and applied transactions/epoch transitions.
The harness never imports expected final-state balances during execution or
rollback replay. The existing reward setter interfaces remain available to
explicit callers, but are not called by the harness. Credential-aware callers
can use `RewardAccountBalanceSetter` with `ledger.RewardAccountKey` to preserve
both credential type and hash.
