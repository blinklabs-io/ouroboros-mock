...
func TestRollbackScenarioPreservesOrder(t *testing.T) {
	blocks, err := fixtures.GenerateConwayChainWithTransactions(1, common.Blake2b256{}, 10, 10, 1)
	if err != nil {
		t.Fatal(err)
	}
	seq := RollbackScenarioEvents(blocks, ocommon.Point{Slot: 42, Hash: []byte{1, 2, 3}}, 764824073)
	if len(seq.Events) != 3 || seq.Events[0].Type != TypeBlock || seq.Events[1].Type != TypeTransaction || seq.Events[2].Type != TypeRollback {
	...
	if txPayload.BlockHash != blocks[0].Hash().String() || len(txPayload.TransactionCbor) == 0 {
